...
# 相较于1版本 不同的是我们将dropout与group添加到每一层卷积中
...
class EncoderSpin(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv1 = gnn.GraphConv(2, 16)
        self.conv2 = gnn.GraphConv(16, 32)
        self.conv3 = gnn.GraphConv(32, 64)
        self.conv4 = gnn.GraphConv(64,128)
        self.conv5 = gnn.GraphConv(128,256)
        self.conv_mu = gnn.GraphConv(256, 256)
        self.conv_logVar = gnn.GraphConv(256, 256)
        self.relu = nn.ReLU()
        self.dropout = nn.Dropout(p=0.5)
        self.batch_norm1 = gnn.BatchNorm(16)
        self.batch_norm2 = gnn.BatchNorm(32)
        self.batch_norm3 = gnn.BatchNorm(64)
        self.batch_norm4 = gnn.BatchNorm(128)
        self.batch_norm5 = gnn.BatchNorm(256)
        self.graph_norm1 = gnn.GraphNorm(16)
        self.graph_norm2 = gnn.GraphNorm(32)
        self.graph_norm3 = gnn.GraphNorm(64)
        self.graph_norm4 = gnn.GraphNorm(128)
        self.graph_norm5 = gnn.GraphNorm(256)
    ...
    def forward(self, x, edge_index, edge_weight, batch):
        ...
        x = self.relu(self.conv4(x, edge_index, edge_weight))
        x = self.batch_norm4(x)
        x = self.graph_norm4(x,batch)
        x = self.dropout(x)
        x = self.relu(self.conv5(x, edge_index, edge_weight))
        x = self.batch_norm5(x)
        x = self.graph_norm5(x,batch)
        x = self.dropout(x)
...
class SVGAE(VGAE):
    ...
    def recon_loss(self, x, x_, **kwargs):
